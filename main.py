...
class DataExtractor:
    ...
    def __init__(self, max_length=None, verbose=0):
        start = timer()
        if verbose:
...
def overall_accuracy_by_class(cm, tags):
    ...
    for index, result in enumerate(diag_values):
        print("{}: {:.2f}%".format(tags[index], result * 100))


def main():
    # c, f = getVectorizedData()
    max_length = 248
    data_extractor = DataExtractor(max_length=max_length, verbose=2)
    # embedding_size = 130
    ...
    # )
    lstm_file = f'lstm_v2_embbedings_{epochs}e'
    # lstm.save(lstm_file)
